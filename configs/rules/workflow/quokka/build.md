...
- **Profiling:** set `tiny_profiler.enabled = 1` to get AMReX's TinyProfiler summary at finalize: per-region time breakdown, including communication routines (e.g. ghost-cell fill), aggregated across MPI ranks. This is the only way to measure actual comm-vs-compute overhead; the always-printed "Performance figure-of-merit" (Mupdates/s) is throughput only, no breakdown. Off by default in past paper/comparison runs to avoid the profiler's own per-region timing overhead skewing that throughput number; enable it deliberately for a dedicated diagnostic run, not one being used for a clean scaling comparison.
